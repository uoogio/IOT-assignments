...
static int cmd_pubdata(int argc, char **argv)
{
//  int temperature = 0;
...
/* int uppert = 50;
 ...
 (uppert - lowert + 1)) + lowert;
 int upperh = 100;
 int lowerh= 0;
  int humidity = (rand() %
 (upperh - lowerh + 1)) + lowerh;
 int upperw = 360;
 int lowerw= 0;
  int winddirection = (rand() %
 (upperw - lowerw + 1)) + lowerw;
char frase[50];
sprintf(frase, "temperature: %d",temperature); */
...
char frase[200];
  ...
  /* step 2: publish data */
  if (emcute_pub(&t, frase, strlen(frase), flags) != EMCUTE_OK) {
      printf("error: unable to publish data to topic '%s [%i]'\n",
              t.name, (int)t.id);
      return 1;
  }
